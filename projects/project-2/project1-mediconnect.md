...
- **Served at:** `http://localhost:3000` (simple HTTP server or nginx container)
- **API Base URL:** `http://localhost:8080` (API Gateway)
- **Auth storage:** JWT stored in `localStorage`; sent as `Authorization: Bearer <token>` header on every authenticated request

### Folder Structure
```
frontend/
├── index.html                  ← shell page with ng-view
├── app.js                      ← angular module, routes ($routeProvider)
├── services/
│   ├── auth.service.js         ← login, register, logout, JWT decode
│   └── api.service.js          ← $http wrapper with auth header injection
├── controllers/
│   ├── auth.controller.js      ← LoginCtrl, RegisterCtrl
│   ├── admin.controller.js     ← AdminCtrl
│   ├── patient.controller.js   ← PatientCtrl
│   └── doctor.controller.js    ← DoctorCtrl
└── views/
    ├── login.html
    ├── register.html
    ├── admin/
    │   └── dashboard.html      ← manage patients, doctors
    ├── patient/
    │   ├── dashboard.html      ← browse doctors, my appointments
    │   ├── doctors.html        ← doctor listing with search/filter
    │   ├── book.html           ← book appointment form
    │   └── payment.html        ← payment form (card input)
    └── doctor/
        ├── profile.html        ← view & edit own profile, availability
        └── appointments.html   ← list + confirm/complete/cancel
```
...
|---|---|---|
| Login | `#!/login` | Username + password form; on success store JWT, redirect by role |
| Register | `#!/register` | Toggle between Patient / Doctor form fields |

**Patient (CUSTOMER role):**
| Page | Route | Description |
|---|---|---|
| Doctor Listing | `#!/doctors` | Cards showing name, specialization, fee, rating; search & filter by specialization |
| Book Appointment | `#!/doctors/:id/book` | Date picker, time input, symptoms textarea |
| My Appointments | `#!/appointments` | Table with status badge; Pay button for CONFIRMED+PENDING_PAYMENT |
| Payment | `#!/appointments/:id/pay` | Card number, expiry, CVV, cardholder name |

**Doctor (SUPPLIER role):**
| Page | Route | Description |
|---|---|---|
| My Profile | `#!/doctor/profile` | Editable form for fee, address, qualifications, availability |
| My Appointments | `#!/doctor/appointments` | Table; Confirm / Complete (with notes) / Cancel actions |

**Admin:**
| Page | Route | Description |
|---|---|---|
| Admin Dashboard | `#!/admin` | Tabbed view: Patients tab + Doctors tab |
| Patients | (tab) | List with Enable / Disable / Delete actions |
| Doctors | (tab) | List with Enable / Disable / Delete actions |

### API Integration Notes
- On login/register success, decode the JWT payload (`atob(token.split('.')[1])`) to read `role`, `userId`, `username`
- Use AngularJS `$routeProvider` to guard routes — redirect to `#!/login` if no token found in `localStorage`
- All `$http` calls go to `http://localhost:8080/api/...`
- Display a friendly error message when the backend returns 4xx/5xx
...
| Criteria | Points | Description |
|---|---|---|
| Auth Flow | 3 | Login and Register (Patient + Doctor) work; JWT stored in localStorage; role-based redirect after login |
| Patient Flows | 4 | Browse/search doctors; book appointment; view own appointments; initiate payment with card details |
| Doctor Flows | 3 | View and update own profile/availability; list appointments; confirm, complete (with notes), cancel |
| Admin Dashboard | 2 | List patients and doctors; enable/disable/delete actions functional |
| UI Quality | 3 | Correct AngularJS 1.x patterns (`ng-repeat`, `ng-if`, `ng-model`, `$http`); no hardcoded API tokens; route guards redirect unauthenticated users to login |
