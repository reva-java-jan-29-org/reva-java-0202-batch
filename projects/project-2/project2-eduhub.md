...
└── views/
    ...
    │   └── dashboard.html
    ├── student/
    │   ├── catalog.html        ← browse published courses
    │   ├── course-detail.html  ← modules list (after enrollment)
    │   ├── my-enrollments.html
    │   └── payment.html
    └── instructor/
        ├── my-courses.html
        ├── course-form.html    ← create/edit course
        └── modules.html        ← add/edit/delete modules
```
...
|---|---|---|
| Login | `#!/login` | Standard login; redirect by role on success |
| Register | `#!/register` | Toggle: Student fields vs Instructor fields |

**Student (CUSTOMER):**
| Page | Route | Description |
|---|---|---|
| Course Catalog | `#!/courses` | Cards with title, instructor, price, rating; search and category filter |
| Course Detail | `#!/courses/:id` | Full description, modules list (locked if not enrolled); Enroll button |
| My Enrollments | `#!/enrollments` | List with status, progress bar; Pay / Update Progress buttons |
| Payment | `#!/enrollments/:id/pay` | Card form |

**Instructor (SUPPLIER):**
| Page | Route | Description |
|---|---|---|
| My Courses | `#!/instructor/courses` | Draft + published courses; Publish / Unpublish / Delete |
| Create/Edit Course | `#!/instructor/courses/new` | Course details form |
| Modules | `#!/instructor/courses/:id/modules` | Add / Edit / Delete modules; shows approval status warning if unapproved |

**Admin:**
| Page | Route | Description |
|---|---|---|
| Admin Dashboard | `#!/admin` | Students tab + Instructors tab; Approve Instructor action |

### API Integration Notes
- Decode JWT payload to get `role`, `userId`, `username` after login
- Route guards: redirect to `#!/login` if no token
- Show "Instructor not yet approved" banner on Instructor dashboard if approval is pending
...
| Criteria | Points | Description |
|---|---|---|
| Auth Flow | 3 | Login and Register (Student + Instructor) work; JWT stored in localStorage; role-based redirect |
| Student Flows | 4 | Browse/search courses; enroll; pay; update learning progress toward completion |
| Instructor Flows | 3 | Create/edit courses; add/delete modules; publish/unpublish; see approval status |
| Admin Dashboard | 2 | List students and instructors; approve instructor; enable/disable/delete actions |
| UI Quality | 3 | Correct AngularJS 1.x patterns; progress bar for enrollment; route guards redirect unauthenticated users to login |
