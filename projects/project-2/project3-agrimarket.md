...
└── views/
    ...
    │   └── dashboard.html
    ├── buyer/
    │   ├── catalog.html        ← browse produce, filter by category
    │   ├── produce-detail.html
    │   ├── cart.html           ← review items before ordering
    │   ├── my-orders.html
    │   └── payment.html
    └── farmer/
        ├── my-listings.html
        ├── listing-form.html   ← create/edit produce listing
        └── orders.html         ← view orders, update delivery status
```
...
| Register | `#!/register` | Toggle: Buyer fields vs Farmer fields |

**Buyer (CUSTOMER):**
| Page | Route | Description |
|---|---|---|
| Produce Catalog | `#!/produce` | Cards with name, farm, price/unit, organic badge; category & search filter |
| Produce Detail | `#!/produce/:id` | Full detail; quantity input; Add to Order button |
| Cart / Order | `#!/cart` | List items with quantities and subtotals; Place Order button |
| My Orders | `#!/orders` | List with status badge; Pay button for PENDING orders |
| Payment | `#!/orders/:id/pay` | Card form |

**Farmer (SUPPLIER):**
| Page | Route | Description |
|---|---|---|
| My Listings | `#!/farmer/listings` | Table of own produce; Edit / Update Stock / Delete |
| Create/Edit Listing | `#!/farmer/listings/new` | Produce form with all fields |
| Orders | `#!/farmer/orders` | Orders containing own produce; Confirm / Pack / Ship / Deliver actions |

**Admin:**
| Page | Route | Description |
|---|---|---|
| Admin Dashboard | `#!/admin` | Buyers tab + Farmers tab; Verify Farmer action |

### API Integration Notes
- Decode JWT payload for `role`, `userId` after login
- Route guards redirect unauthenticated users to `#!/login`
- Show "Account not verified" warning on Farmer dashboard if not yet verified by Admin
...
| Criteria | Points | Description |
|---|---|---|
| Auth Flow | 3 | Login and Register (Buyer + Farmer) work; JWT stored in localStorage; role-based redirect |
| Buyer Flows | 4 | Browse/filter produce; build a multi-item order; pay for order with card form |
| Farmer Flows | 3 | Create/edit produce listings; update stock; view orders for own produce; advance delivery status |
| Admin Dashboard | 2 | List buyers and farmers; verify farmer; enable/disable/delete actions |
| UI Quality | 3 | Correct AngularJS 1.x patterns; multi-item order list with running total; route guards redirect unauthenticated users |
