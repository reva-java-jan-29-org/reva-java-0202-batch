...
└── views/
    ...
    │   └── dashboard.html
    ├── owner/
    │   ├── garages.html        ← browse garages with services
    │   ├── garage-detail.html  ← services list, Book button
    │   ├── book.html           ← booking form
    │   ├── my-bookings.html
    │   └── payment.html
    └── mechanic/
        ├── my-garage.html      ← view & edit garage profile, hours
        ├── services.html       ← add/edit/delete service offerings
        └── bookings.html       ← list + Confirm/Start/Complete + add parts
```
...
| Register | `#!/register` | Toggle: Vehicle Owner fields vs Mechanic/Garage fields |

**Vehicle Owner (CUSTOMER):**
| Page | Route | Description |
|---|---|---|
| Garage Listing | `#!/garages` | Cards: name, location, specializations, rating; search filter |
| Garage Detail | `#!/garages/:id` | Services table with duration and base price; Book button per service |
| Book Service | `#!/garages/:id/book` | Date, time, service selector, problem description |
| My Bookings | `#!/bookings` | List with status + payment badge; Pay button when COMPLETED |
| Payment | `#!/bookings/:id/pay` | Card form; shows finalAmount if set, else estimatedAmount |

**Mechanic (SUPPLIER):**
| Page | Route | Description |
|---|---|---|
| My Garage | `#!/mechanic/garage` | Edit profile, working hours, active days |
| Service Offerings | `#!/mechanic/services` | Add/Edit/Delete services |
| Bookings | `#!/mechanic/bookings` | List; Confirm / Start Work / Complete (with final amount + notes) / Add Part actions |

**Admin:**
| Page | Route | Description |
|---|---|---|
| Admin Dashboard | `#!/admin` | Vehicle Owners tab + Mechanics tab; Verify Mechanic action |

### API Integration Notes
- Decode JWT payload for `role`, `userId` after login
- Route guards redirect unauthenticated users to `#!/login`
- Show "Garage not verified" warning on Mechanic dashboard if not yet verified
...
| Criteria | Points | Description |
|---|---|---|
| Auth Flow | 3 | Login and Register (Vehicle Owner + Mechanic) work; JWT stored in localStorage; role-based redirect |
| Owner Flows | 4 | Browse garages; view services; book a service slot; view bookings; pay (shows final or estimated amount) |
| Mechanic Flows | 3 | Edit garage profile/hours; manage service offerings; advance booking status; add parts during IN_PROGRESS |
| Admin Dashboard | 2 | List vehicle owners and mechanics; verify mechanic; enable/disable/delete actions |
| UI Quality | 3 | Correct AngularJS 1.x patterns; parts list rendered within booking detail; route guards redirect unauthenticated users |
